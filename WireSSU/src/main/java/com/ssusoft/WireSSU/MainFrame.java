	...
	public static int sizeX, sizeY;//윈도우 가로, 세로
	   ...
	   MainFrame(){
	     Dimension dim = Toolkit.getDefaultToolkit().getScreenSize(); //화면 크기 구하기
	      ...
	      icon =new ImageIcon("D:\\Haeri_Data\\2019 1학기\\SoftwareProject\\worldmap.png"); //지도 사진 가져오기
	      
	      panel =new JPanel() {
	         public void paintComponent(Graphics g) {
	             g.drawImage(icon.getImage(), 0, 0, sizeX, sizeY, null);   //이미지 크기 정하기
	             setOpaque(false);
...
//	      makeButton(panel, xpos*3, ypos*3, "me", false);//false 일 경우 분홍색
	   ...
	   public static void makeButton(JPanel panel, int x, int y, String s, boolean bool) {
		      ImageIcon imgc = null;
		      if(bool==true) {
		         imgc = new ImageIcon("D:\\Haeri_Data\\2019 1학기\\SoftwareProject\\pinkdot.png");
		      }
		      else {
